...
def upload():
    ...
    if request.method == 'POST':
        print(f'[UPLOAD] cover={request.files.get("cover")}, audio={request.files.get("audio")}, cover_size={request.files.get("cover").content_length if request.files.get("cover") else 0}', flush=True)
